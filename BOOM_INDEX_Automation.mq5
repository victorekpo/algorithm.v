...
bool shouldAllowFromRSI[6] = {true, true, true, true, true, true};
...
// calcRSI5History();
   ...
   steps = getStepsLastBar(PERIOD_M1,0,true);
...
// RSI checks using predefined thresholds
   ...
   int rsiThresholds[6] = {30, 30, 30, 30, 30, 30};

   for(int i = 0; i < ArraySize(rsiValues); i++)
     {
      if(rsiValues[i] < rsiThresholds[i])
        {
         shouldAllowFromRSI[i] = false;
        }
      else
         if(rsiValues[i] > 70)
           {
            shouldAllowFromRSI[i] = true;
           }
     }

// Pending orders cleanup if account equity or profit is low
   if(calculateOrders() >= 1
      && (AccountInfoDouble(ACCOUNT_PROFIT) < -300 || AccountInfoDouble(ACCOUNT_EQUITY) < 500))
     {
   ...
   if(
   ...
   )
     {
      if(criticalConditionsForBuyOrder())
        {
         orderPlaced = OrderSend(_Symbol, OP_BUY, lotSize, Ask, 3, 0, 0, "NEW_algorithm.V_BUY_" + comment, magic, 0, Blue);
...
int placeBuyStopOrder(double lotSize, string comment, int magic, int gapAmt = 1)
  {
   ...
   if(
   ...
   )
     {
      if(criticalConditionsForBuyOrder())
        {
         orderPlaced = OrderSend(_Symbol, OP_BUYSTOP, lotSize, price, 3, 0, 0, "NEW_algorithm.V_BUYSTOP_" + comment, magic, 0, Blue);
...
int placeBuyLimitOrder(double lotSize, string comment, int magic, int gapAmt = 1, double orderPrice = 0)
  {
   ...
   if(
   ...
   )
     {
      if(criticalConditionsForBuyOrder())
        {
         orderPlaced = OrderSend(_Symbol, OP_BUYLIMIT, lotSize, price, 3, 0, 0, "NEW_algorithm.V_BUYLIMIT_" + comment, magic, 0, Blue);
   ...
   if(
   ...
   )
     {
      if(criticalConditionsForSellOrder())
        {
         orderPlaced = OrderSend(_Symbol, OP_SELL, lotSize, Bid, 3, 0, 0, "NEW_algorithm.V_SELL_" + comment, magic, 0, Red);
...
int placeSellStopOrder(double lotSize, string comment, int magic, int gapAmt = 1)
  {
   ...
   if(
   ...
   )
     {
      if(criticalConditionsForSellOrder())
        {
         orderPlaced = OrderSend(_Symbol, OP_SELLSTOP, lotSize, price, 3, 0, 0, "NEW_algorithm.V_SELLSTOP_" + comment, magic, 0, Red);
...
int placeSellLimitOrder(double lotSize, string comment, int magic, int gapAmt = 1, double orderPrice = 0)
  {
   ...
   if(
   ...
   )
     {
      if(criticalConditionsForSellOrder())
        {
         orderPlaced = OrderSend(_Symbol, OP_SELLLIMIT, lotSize, price, 3, 0, 0, "NEW_algorithm.V_SELLLIMIT_" + comment, magic, 0, Red);
...
//|                                                                  |
//+------------------------------------------------------------------+
int getStepsLastBar(int time, int period = 0, bool bull = true)
  {
   int count = 1;
   while(true)
     {
      bool bar = bull ? isThisBarBULLISH(time, period) : isThisBarBEARISH(time, period);
      if(bar && getBarSize(time, period) > 20)
        {
         return count;
        }
      count++;
...
bool allMAUp()
  {
   return checkMATrend(true);  // true indicates checking for "up" trend
  }
...
bool allMADown()
  {
   return checkMATrend(false);  // false indicates checking for "down" trend
  }

//+------------------------------------------------------------------+
//| Helper function to check if all relevant MAs are in the same trend |
//+------------------------------------------------------------------+
bool checkMATrend(bool isUp)
  {
...
//+------------------------------------------------------------------+
int checkOccurenceSell(int time = 1, int periods = 1000)
  {
   int calc = 0;
...
//+------------------------------------------------------------------+
int checkOccurenceBuy(int time = 1, int periods = 1000)
  {
   int calc = 0;
...
double calculateLots()
  {
   ...
   int rowSize = 6;
   return getEquityBasedValue(lotThresholds, equity, rowSize);
  }
...
//                && AccountInfoDouble(ACCOUNT_EQUITY) > -500;
//
//            if (canDoubleUp) {
...
bool ShouldCloseOrderEarly(long ticket)
  {
   ...
   bool closeByTime = currentTime > timeAlarm;
...
void CheckForEarlyClose()
  {
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      if(!m_position.SelectByIndex(i) || isHODLOrder(m_position.Comment()))
         continue;
      ...
      if(ShouldCloseOrderEarly(ticket) && m_trade.PositionClose(ticket))
        {
   ...
   bool shouldTakeMoreReversalBuy =
      ...
      && (
         ...
         || (
            ...
            && getRSI(H1) < 40
            && (
               AccountInfoDouble(ACCOUNT_MARGIN_LEVEL) > 10000
               || numberOfBuyOrders < 1
            )
         )
      );
      ...
      if(m_position.SelectByIndex(i) && m_position.Symbol() == _Symbol)
        {
         ...
         if(currentProfit < -(3 * m_position.Volume()))
           {
...
bool checkAccountHealth(ENUM_POSITION_TYPE type)
  {
   int equityThreshold = 500;
   int marginThreshold = 5000;
   ...
   return (((calculateOrdersByTime(timeThreshold, type, true) < (type == POSITION_TYPE_BUY ? maxBuyOrders : maxSellOrders)
            ...
            && (checkAccountEquity(equityThreshold) || checkAccountMargin(marginThreshold))
...
bool RSIConditionsForSellOrder()
  {
   return (
          ...
          && getRSI(W1) > 35
          && getRSI(M15) > 50
          && shouldAllowFromRSI[M5]
          && shouldAllowFromRSI[M1];
...
// Define arrays for multiple period checks
   ...
   return getMA(M1, _20) > getHighestPrice(M1, 1)
...
// && getMA(M1, _20) < getMA(M1, _50)
          ...
          && (
             getMA(M1, _500) > getHighestPrice(M1, 1)
             || checkAccountEquity(2000)
          )
          && (
          ...
          && (
             getMA(M30, _9) > getHighestPrice(M30, 1)
             || getMA(M30, _20) > getHighestPrice(M30, 1)
          )
          && (
...
void CheckForOpenMAConditions()
  {
   if(
     ...
     {
      placeSellOrder(orderLots, "MA", 100);
     }
  }
...
void CheckForOpenRSIConditions()
  {
   if(
     ...
     {
      placeSellOrder(orderLots, "RSI", 200);
     }
  }
...
void CheckForOpenMARSIConditions()
  {
   if(
     ...
     {
      placeSellOrder(orderLots, "MARSI", 300);
     }
  }
